...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/16 17:08:39 by skeveyan          #+#    #+#             */
/*   Updated: 2022/08/16 17:19:34 by skeveyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0
		...
		return (NULL);
	if ((read_chek(fd, &temporary[fd], &read_value)))
		return (NULL);
	if (0 > (write_return(&temporary[fd], &returnline, &i_return)))
		return (NULL);
	if (!(take_temporary(&temporary[fd], &i_return)))
		return (returnline);
	return (NULL);
}
