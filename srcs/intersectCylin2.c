...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/10 00:16:40 by skeveyan          #+#    #+#             */
/*   Updated: 2023/07/10 00:16:43 by skeveyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minirt.h"
...
t_vec	cylinder_normal2(t_cylinder cylin, t_cross cross, float m)
{
	...
	norm = normalize(vecSub(vecSub(cross.p, topCapCenter(cylin)),
				vecScale(m, cylin.axis)));
	return (norm);
}
...
bool	intersectInfiniteCylin(t_ray ray, t_cylinder cylin, t_cross *cross)
{
	...
	c = dotProduct(X, X) - pow(dotProduct(X, cylin.axis), 2)
		- cylin.radius * cylin.radius;
	b = 2 * (dotProduct(ray.dir, X) - dotProduct(ray.dir, cylin.axis)
			* dotProduct(X, cylin.axis));
...
bool	intersectCylin2(t_ray ray, t_cylinder cylin, t_cross *cross)
{
	...
	if (intersectPlane(ray, plane, &crossPlane))
	{
		...
		if (vecNorm(vecSub(hitPoint, cylin.topCapCent)) <= cylin.radius
			&& crossPlane.t < t)
			t = crossPlane.t;
	}
